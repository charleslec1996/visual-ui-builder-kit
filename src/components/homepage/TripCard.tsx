...
const TripCard: React.FC<TripCardProps> = ({ trip }) => {
  return (
    <article className="bg-white self-stretch flex min-w-60 min-h-[682px] flex-col overflow-hidden items-stretch justify-center grow shrink w-[307px] my-auto p-6 rounded-[32px] max-md:px-5">
      <div className="max-w-full w-[336px] rounded-[0px_0px_0px_0px]">
        <img
          src={trip.image}
          className="aspect-[0.8] object-contain w-full rounded-[0px_0px_0px_0px]"
          alt={trip.name}
        />
      </div>
      <div className="w-[279px] max-w-full text-black tracking-[0.5px] mt-4">
        <div className="flex w-full flex-col items-stretch text-base font-bold justify-center">
          <div className="bg-[rgba(138,244,169,1)] gap-3 p-2 rounded-[120px] text-center">
            {trip.className}
          </div>
          <h3 className="text-2xl leading-8 mt-3">
            {trip.name}
          </h3>
          <div className="text-[rgba(131,93,254,1)] font-semibold text-right mt-3">
